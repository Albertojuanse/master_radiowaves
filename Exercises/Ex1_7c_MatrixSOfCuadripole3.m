...
% -> ZL1 = ZL2 = 0 y 
% -> ZL1 = ZL2 = infinito,
% concatenando matrices ABCD y dibujar su respuesta en frecuencia.
...
for i_f = 1:numel(vf) 
    ...
    lb = l0;
    beta_b = beta0;
    ABCDZ01 = ABCDofLine(Zb, beta_b(i_f), lb);

    %% Primer stub
    Zx = Z1*(ZL1 + 1j*Z1*tan(beta1(i_f)*l1))/(Z1 + 1j*ZL1*tan(beta1(i_f)*l1));
    Yx = 1/Zx;
    ABCDY1 = ABCDofAdmitance(Yx);

    %% Línea central
    ABCDZc = ABCDofLine(Zc, beta_c(i_f), lc);

    %% Segundo stub
    Zx = Z2*(ZL2 + 1j*Z2*tan(beta2(i_f)*l2))/(Z2 + 1j*ZL2*tan(beta2(i_f)*l2));
    Yx = 1/Zx;
    ABCDY2 = ABCDofAdmitance(Yx);

    %% Segunda borna
    Zb = Z0;
    lb = l0;
    beta_b = beta0;
    ABCDZ02 = ABCDofLine(Zb, beta_b(i_f), lb);
    
    %% Total
    ABCDTotal = ABCDZ01*ABCDY1*ABCDZc*ABCDY2*ABCDZ02;
    STotal = ABCDtoS(ABCDTotal, Z0, Z0);
...
figure(1);
plot(vf, 10*log10(abs(S11T)));
hold on;
plot(vf, 10*log10(abs(S12T)));
hold off;
...
figure(2);
plot(vf, 10*log10(abs(S21T)));
hold on;
plot(vf, 10*log10(abs(S22T)));
hold off;
