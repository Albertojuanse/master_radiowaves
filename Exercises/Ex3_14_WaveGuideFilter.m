...
% Calcular la respuesta del apartado 13 para Q = {100, 1000, 10 000},
...
for each_Q = Q
    ...
    alpha = pi/(2*each_Q*(lambda0/2));
    
    for i_pulsation = 1:m
        ABCD_LT1 = ABCDofLossyLine(Zc, beta(i_pulsation), lambda0/2, alpha);
        ABCD_LT2 = ABCDofLossyLine(Zc, beta(i_pulsation), lambda0/2, alpha);
        ABCD_LT3 = ABCDofLossyLine(Zc, beta(i_pulsation), lambda0/2, alpha);
        ABCD_LT4 = ABCDofLossyLine(Zc, beta(i_pulsation), lambda0/2, alpha);
%         ABCD_LT5 = ABCDofLossyLine(Zc, beta(i_pulsation), lambda0/2, alpha);
